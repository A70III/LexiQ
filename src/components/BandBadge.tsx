...
export default function BandBadge({ band, size = "md" }: BandBadgeProps) {
  const sizeClass = size === "sm" ? "text-xs px-1.5 py-0.5" : size === "lg" ? "text-lg px-3 py-1.5" : "text-sm px-2 py-1";
  return (
    <span
      className={`inline-flex items-center gap-1 rounded-full font-bold bg-gradient-to-br ${bandColor(band)} ${sizeClass}`}
      style={{
